...
export function transferLoad(sourceBattenId: string, pieceId: string, targetBattenId: string) {
  return request<TransferResult>(
    `/api/battens/${encodeURIComponent(sourceBattenId)}/transfers`,
    {
      method: 'POST',
      body: JSON.stringify({ piece_id: pieceId, target_batten_id: targetBattenId }),
    },
  );
}
